...
type Callbacks struct {
	...
	// done (no other errors are returned). If the host is not awake, it is
	// awoken if wake is true. While the lock is held, the host is not allowed
	// go to sleep (it is not guaranteed that host will be available in the
	// entire time the lock is held). The returned function must be called to
	// unlock the lock. The host will not necessarily sleep immediately after
